...
## Pitfall: prepare-server.sh uses relative `../../app` → Permission denied on NAS

A helper script like `scripts/prepare-server.sh` that references the target with `../../app/...` only works when run from a specific cwd. Run from the repo root (the normal `bash scripts/prepare-server.sh`), `../../app` resolves to the **parent's parent** of the repo — `mkdir` there fails with `Permission denied`, and the build silently breaks.

**Fix — derive the repo root from the script's own location, never from cwd:**
```bash
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
# then use "${REPO_ROOT}/app/server" everywhere, and quote it (paths may contain spaces)
```
Also remember: on the NAS the node/npm from App Center (`/vol4/@appcenter/nodejs_v24/bin`) is **not** on PATH — export it before running `npm pack` inside the script.
