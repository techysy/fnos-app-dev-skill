...
- **v0.3.1**: ✅ WORKING — verified installable, no `install_dep_apps` in manifest（2026-07 历史验证；2026-08-08 已确认 `install_dep_apps=nodejs_v24` 是有效依赖声明，见下方 Key）
...
**Key**: 该历史版本（v0.3.1，2026-07）未用 `install_dep_apps` 字段（当时 fnOS 版本可能不支持）。**2026-08-08 已实测 `install_dep_apps=nodejs_v24` 有效**（9Router v0.5.51：fnOS 自动安装 nodejs_v24 并正常启动）。若你的 fnOS 版本仍拒绝，先检查 manifest 格式（KEY=VALUE 等号两侧空格），再考虑退回无依赖字段 + `cmd/install_init` 手动处理。
...
**Problem**: Setting `no_proxy="localhost,127.0.0.1,192.168.1.*"` doesn't work with Python's `requests` library. The wildcard `*` is not supported. Requests to `<NAS_IP>:8642` get sent through the proxy, causing 501 errors or wrong HTTP method concatenation.
...
2. Use explicit IPs: `export no_proxy="localhost,127.0.0.1,<NAS_IP>,<NAS_IP>"`

### cmd/main Must Be Finalized Before Building fpk

After `fnpack build` and installation, `/var/apps/{appname}/cmd/` directory and files are owned by **root**. You cannot modify cmd/main from the <user> user without sudo.
...
```bash
tar czf - -C ~/.hermes skills/ | ssh <user>@<NAS_IP> "tar xzf - -C /vol4/@appdata/HermesWebUI/hermes_home"
```
...
REMOTE_GATEWAY="${GATEWAY_URL:-http://<NAS_IP>:8642}"
REMOTE_DASHBOARD="${DASHBOARD_URL:-http://<NAS_IP>:9119}"
